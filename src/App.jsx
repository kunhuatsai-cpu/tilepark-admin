...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // 加入 maximum-scale=1.0, user-scalable=no 確保不被意外縮放
    meta.content = "width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no";

    // 🛠️ 2. Tailwind CSS 嚴格檢查
    const checkTailwind = () => {
      if (window.tailwind) {
        setStyleLoaded(true);
      } else {
  ...
  return (
    <div className="h-screen w-full bg-gray-50 font-sans text-gray-800 flex flex-col md:flex-row overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className="bg-[#222] text-white flex-shrink-0 flex flex-col md:w-64 z-20 shadow-lg md:shadow-none hidden md:flex">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
            <div className="w-full sm:w-auto flex justify-between items-center">
              ...
              <button 
                ...
                className="md:hidden ml-2 text-xs text-gray-400 hover:text-red-500 underline decoration-dotted whitespace-nowrap"
              >
                登出
              </button>
            </div>
            ...
            <div className="flex w-full sm:w-auto gap-2">
              ...
              <button 
                onClick={() => setIsLoggedIn(false)}
                className="hidden md:block ml-2 text-xs text-gray-400 hover:text-red-500 underline decoration-dotted whitespace-nowrap"
              >
                登出
              </button>
            </div>
